      ...
      DO KAZ=-KVCZ2,MAX(0,KVCZ2-1)
       ...
       DO MOJ=ICORE+1,IOCC
        DO MOA=IOCC+1,IALL(KAX,KAY,KAZ)-IVIRTCORE
         DO MOI=ICORE+1,IOCC
         ! IF((EPSILON(MOI,KIX*NMOD,KIY*NMOD,KIZ*NMOD)+EPSILON(MOJ,KJX*NMOD,KJY*NMOD,KJZ*NMOD) &
...
!  INTEGER :: KJOBS,KJOBE
   ...
   DOUBLE PRECISION,ALLOCATABLE :: QEPSILON_TMP(:,:,:,:)
   INTEGER :: KVCX2,KVCY2,KVCZ2,LINIDXI,LINIDXJ,LINIDXA,LINIDXB
   INTEGER :: NMOD
   INTEGER :: KVC2
   INTEGER,ALLOCATABLE :: KVC3(:)
   LOGICAL,ALLOCATABLE :: LKVC(:,:)
   DOUBLE PRECISION :: NKPTOTAL
   DOUBLE PRECISION :: TEMPMP, MU, BETA
   DOUBLE PRECISION :: NBAR
   DOUBLE PRECISION :: NORMDIRAC
   DOUBLE PRECISION :: TEMPORARY
   DOUBLE PRECISION :: EI,EJ,EA,EB,D,FI,FJ,FA,FB
   DOUBLE PRECISION :: ERRORTRAP
   DOUBLE PRECISION :: MP0_OMEGA,MP0_OMEGA1,MP0_U,MP0_MU
   DOUBLE PRECISION :: MP1_OMEGA,MP1_U,MP1_MU
   DOUBLE PRECISION :: MP2_OMEGA,MP2_U,MP2_MU,SIGMA_H,SIGMA_L
   DOUBLE PRECISION :: MP1_OMEGA_1E, MP1_OMEGA_1E_TMP, MP1_OMEGA_2E, MP1_OMEGA_2E_TMP
   DOUBLE PRECISION :: MP1_U_1E, MP1_U_1E_TMP, MP1_U_2E, MP1_U_2E_TMP
   DOUBLE PRECISION :: MP1_MU_1E, MP1_MU_1E_TMP
   DOUBLE PRECISION :: MP2_OMEGA_1E, MP2_OMEGA_1E_TMP, MP2_OMEGA_2E, MP2_OMEGA_2E_TMP
   DOUBLE PRECISION :: MP2_OMEGA_4E, MP2_OMEGA_4E_TMP
   DOUBLE PRECISION :: MP2_U_1E, MP2_U_1E_TMP, MP2_U_2E, MP2_U_2E_TMP, MP2_U_2E_TMP1
   DOUBLE PRECISION :: MP2_U_3E, MP2_U_3E_TMP, MP2_U_4E, MP2_U_4E_TMP
   DOUBLE PRECISION :: MP2_MU_1E, MP2_MU_1E_TMP, MP2_MU_2E, MP2_MU_2E_TMP, MP2_MU_2E_TMP1
   DOUBLE PRECISION :: MP2_MU_3E, MP2_MU_3E_TMP, MP2_MU_4E, MP2_MU_4E_TMP
   DOUBLE PRECISION :: MP2_N1,MP2_N2,MP2_A1,MP2_A2,MP2_K1,MP2_K2,MP2_R1,MP2_R2
   DOUBLE PRECISION,ALLOCATABLE :: KL(:,:), FT(:,:)
   INTEGER :: TICKET
   INTEGER :: NFILES,IFILE,JFILE
   LOGICAL :: LEXIST
   CHARACTER(6) :: FILEAPPENDIX
...
! -- HELIX
   ...
   IF(KVCX==0 .AND. KVCY==0 .AND. KVCZ==1) THEN

   WRITE (6, '(A)') 'ENTERING MOLECULAR BRANCH'
   ...
   ! ALLOCATE(KL(ICORE+1:IALLMAX-IVIRTCORE,ICORE+1:IALLMAX-IVIRTCORE))
   ...
   DO KB=0,0
    DO MOB=ICORE+1,IALL(0,0,0)-IVIRTCORE
     ...
     DO MOJ=ICORE+1,IALL(0,0,0)-IVIRTCORE
      ...
      !   PRINTING TALL
      !   WRITE(*,*) 'TALL(', MOI, MOA, MOJ, MOB, ') = ', &
      !    DREAL(TALL(MOI,MOA,MOJ,MOB)), DIMAG(TALL(MOI,MOA,MOJ,MOB))
       ENDDO
      ENDDO
     ENDDO
    ENDDO
   ...
   ! WRITE(6,'(A)') 'TALL INTEGRALS (REAL, IMAG):'
   ! DO MOI = ICORE+1, IALL(0,0,0)-IVIRTCORE
   ! DO MOJ = ICORE+1, IALL(0,0,0)-IVIRTCORE
   ! DO MOA = ICORE+1, IALL(0,0,0)-IVIRTCORE
   ! DO MOB = ICORE+1, IALL(0,0,0)-IVIRTCORE
   !    WRITE(6,'(A,4I5,A,2X,2F15.8)') 'TALL(', MOI, MOA, MOJ, MOB, ') = ', &
   !    DREAL(TALL(MOI,MOA,MOJ,MOB)), DIMAG(TALL(MOI,MOA,MOJ,MOB))
...
!    ENDDO
   ...
   DO MOI=ICORE+1,IALL(0,0,0)-IVIRTCORE
      FTDM(MOI,MOI)=FTDM(MOI,MOI)-EPSILON(MOI,0,0,0)
   ENDDO


   WRITE(6,'(A,I5)') "IOCC = ", IOCC
   WRITE(6,'(A,I5)') "IVIRTCORE = ", IVIRTCORE

   IF ((IOPTN(9) >= 2).AND.(MYID == 0)) THEN
   WRITE(6,'(A)') 'PRINTING EPSILON:'
   DO MOI=ICORE+1,IALL(0,0,0)-IVIRTCORE
    WRITE(6,'(I5,2X,F20.10)') MOI,EPSILON(MOI,0,0,0)
   ENDDO
   ENDIF

   IF ((IOPTN(9) >= 2).AND.(MYID == 0)) THEN
    WRITE(6,'(A)') 'FTDM MATRIX:'
    DO MOI=ICORE+1,IALL(0,0,0)-IVIRTCORE
     WRITE(6,'(I5,2X)',ADVANCE='NO') MOI
     DO MOJ=ICORE+1,IALL(0,0,0)-IVIRTCORE
      WRITE(6,'(F20.10,2X)',ADVANCE='NO') FTDM(MOI,MOJ)
     ENDDO
     WRITE(6,*)
    ENDDO
   ENDIF
...
! Printing FT for checking in grid structure
   ! WRITE(6,'(A)') 'FT MATRIX:'
   ! DO MOI=ICORE+1,IALL(0,0,0)-IVIRTCORE
   !  WRITE(6,'(I5,2X)',ADVANCE='NO') MOI
   !  DO MOJ=ICORE+1,IALL(0,0,0)-IVIRTCORE
   !   WRITE(6,'(F20.10,2X)',ADVANCE='NO') FT(MOI,MOJ)
   !  ENDDO
   !  WRITE(6,*)
   ! ENDDO
...
! NBAR Calculation
   TEMPMP=DOPTN(98)
   BETA=1.0D0/(BOLTZMANN*TEMPMP)
   MU=FERMI
   WRITE(6,'(A,F20.10,A)') 'TEMP      = ',TEMPMP,' K'
   WRITE(6,'(A,F20.10,A)') 'FERMI     = ',FERMI,' HARTREE'
   WRITE(6,'(A,E20.10E3,A)') 'BETA      = ',BETA,' HARTREE**-1'
   ...
   DO MOI=ICORE+1,IALL(0,0,0)-IVIRTCORE
    ...
    NBAR=NBAR+2.0D0*FI
   ENDDO
   WRITE(6,'(A,F20.15,A)') 'NUMBER OF ELECTRONS = ',NBAR,' ELECTRONS'

   NORMDIRAC=0.0D0
   DO MOI=ICORE+1,IALL(0,0,0)-IVIRTCORE
    FI=1.0D0/(DEXP((EPSILON(MOI,0,0,0)-MU)*BETA)+1.0D0)
    NORMDIRAC=NORMDIRAC+2.0D0*FI*(1.0D0-FI)
   ENDDO

   WRITE(6,'(A,F20.15,A)') 'NORMDIRAC = ',NORMDIRAC,' '

   IF (DABS(NORMDIRAC) < ERRORTRAP) THEN
    MP0_MU=(EPSILON(IOCC,0,0,0)+EPSILON(IOCC+1,0,0,0))/2.0D0
    MU=MP0_MU
    WRITE(6,'(A)') 'NORMDIRAC IS TOO SMALL. SETTING MU TO MIDPOINT OF HOMO-LUMO GAP.'
   ELSE
    MP0_MU=MU
   ENDIF
   ...
   DO MOI=ICORE+1,IALL(0,0,0)-IVIRTCORE
    FI=1.0D0/(DEXP((EPSILON(MOI,0,0,0)-FERMI)*BETA)+1.0D0)
    TEMPORARY=FTDM(MOI,MOI)*2.0D0*FI
    MP1_OMEGA=MP1_OMEGA+TEMPORARY
    MP1_MU=MP1_MU+TEMPORARY*(1.0D0-FI)
    MP1_U=MP1_U+TEMPORARY
   ENDDO
   ...
   IF (DABS(NORMDIRAC) < ERRORTRAP) THEN
    MP1_MU=0.0D0
    WRITE(6,'(A)') 'NORMDIRAC IS TOO SMALL. SETTING MP1_MU TO ZERO.'
   ELSE
    MP1_MU=MP1_MU/NORMDIRAC ! READY
   ENDIF

   TEMPORARY=0.0D0
   DO MOI=ICORE+1,IALL(0,0,0)-IVIRTCORE
    FI=1.0D0/(DEXP((EPSILON(MOI,0,0,0)-FERMI)*BETA)+1.0D0)
    TEMPORARY=BETA*(FTDM(MOI,MOI)-MP1_MU)*2.0D0*FI*(1.0D0-FI)*EPSILON(MOI,0,0,0)
    ! WRITE(*,*) 'MOI=',MOI,' TEMPORARY MU CORRECTION=',TEMPORARY
    MP1_U=MP1_U-TEMPORARY
   ENDDO

   ! WRITE(*,*) 'MP1_U BEFORE TWO-ELECTRON INTEGRALS=',MP1_U
   ! ERRORTRAP=MP1_U

   DO MOJ=ICORE+1,IALL(0,0,0)-IVIRTCORE
    DO MOI=ICORE+1,IALL(0,0,0)-IVIRTCORE
     FI=1.0D0/(DEXP((EPSILON(MOI,0,0,0)-FERMI)*BETA)+1.0D0)
     FJ=1.0D0/(DEXP((EPSILON(MOJ,0,0,0)-FERMI)*BETA)+1.0D0)
     TEMPORARY=(4.0D0*DREAL(TALL(MOI,MOI,MOJ,MOJ))-DREAL(2.0D0*TALL(MOI,MOJ,MOJ,MOI)))
     ! WRITE(*,*) 'MOI=',MOI,' MOJ=',MOJ,' TEMPORARY=',TEMPORARY, &
     ! ' FI=',FI,' FJ=',FJ
     MP1_OMEGA=MP1_OMEGA-0.5D0*TEMPORARY*FI*FJ
     MP1_U=MP1_U-0.5D0*TEMPORARY*FI*FJ
     ! WRITE(*,*) 'MP1_U INTERMEDIATE=',MP1_U
    ENDDO
   ENDDO

   ! WRITE(*,*) 'MP1_U AFTER TWO-ELECTRON INTEGRALS=',MP1_U,' CORRECTION=',MP1_U-ERRORTRAP  


   MP1_OMEGA=MP1_OMEGA-MP1_MU*NBAR
...
! ! Printing EPSILON values for checking

!    WRITE(6,'(A)') 'ORBITAL ENERGIES:'
!    WRITE(6,'(I10)') IALL(0,0,0)
!    WRITE(6,'(A)') ' '
!    WRITE(6,'(I10)') ICORE
!    WRITE(6,'(A)') ' '
!    WRITE(6,'(I10)') IVIRTCORE
!    WRITE(6,'(A)') ' '

!    DO MOI=2*ICORE+1,2*(IALL(0,0,0)-IVIRTCORE)
!     WRITE(6,'(I5,F20.10)') MOI,EPSILON((MOI+1)/2,0,0,0)
!    ENDDO
   ...
   DO MOI=ICORE+1,IALL(0,0,0)-IVIRTCORE
    FI=1.0D0/(DEXP((EPSILON(MOI,0,0,0)-FERMI)*BETA)+1.0D0)
    EI=EPSILON(MOI,0,0,0)
    TEMPORARY=2.0D0*BETA*MP1_MU*MP1_MU*FI*(1.0D0-FI)
    MP2_MU=MP2_MU-TEMPORARY*((1.0D0-FI)-FI)/2.0D0
    MP2_U=MP2_U+BETA*TEMPORARY*(EI*(1.0D0-FI)-EI*FI)/2.0D0

    TEMPORARY=2.0D0*BETA*MP1_MU*FTDM(MOI,MOI)*FI*(1.0D0-FI)
    MP2_MU=MP2_MU+TEMPORARY*((1.0D0-FI)-FI)
    MP2_U=MP2_U-BETA*TEMPORARY*(EI*(1.0D0-FI)-EI*FI)
    DO MOJ=ICORE+1,IALL(0,0,0)-IVIRTCORE
     FJ=1.0D0/(DEXP((EPSILON(MOJ,0,0,0)-FERMI)*BETA)+1.0D0)
     EJ=EPSILON(MOJ,0,0,0)
     D=EI-EJ
     TEMPORARY=2.0D0*FTDM(MOI,MOJ)*FTDM(MOI,MOJ)*FI*(1.0D0-FJ)
     IF(DABS(D)>1.0D-10) THEN
      MP2_MU=MP2_MU+TEMPORARY*((1.0D0-FI)-FJ)/D
      MP2_U=MP2_U-BETA*(EI*(1.0D0-FI)-EJ*FJ)*TEMPORARY/D
     ELSE
      MP2_MU=MP2_MU-TEMPORARY*((1.0D0-FI)-FJ)*BETA/2.0D0
      MP2_U=MP2_U+TEMPORARY*(EI*(1.0D0-FI)-EJ*FJ)*BETA*BETA/2.0D0
     ENDIF
    TEMPORARY=(4.0D0*DREAL(TALL(MOI,MOI,MOJ,MOJ))-2.0D0*DREAL(TALL(MOI,MOJ,MOJ,MOI))) &
     *FI*(1.0D0-FI)*FJ*(1.0D0-FJ)*BETA*MP1_MU
    MP2_MU=MP2_MU+TEMPORARY
    MP2_U=MP2_U-BETA*TEMPORARY*EJ
    DO MOA=ICORE+1,IALL(0,0,0)-IVIRTCORE
    FA=1.0D0/(DEXP((EPSILON(MOA,0,0,0)-FERMI)*BETA)+1.0D0)
    EA=EPSILON(MOA,0,0,0)
    D=EI-EJ
    ! WRITE(*,*) 'MOI=',MOI,' MOJ=',MOJ,' MOA=',MOA,' TALL (ONLY REAL)', &
    ! DREAL(TALL(MOI,MOJ,MOA,MOA)), ' ', DREAL(TALL(MOI,MOA,MOA,MOJ)), &
    ! ' ', DREAL(TALL(MOJ,MOI,MOA,MOA)), ' ', DREAL(TALL(MOJ,MOA,MOA,MOI))
    TEMPORARY=4.0D0*TALL(MOI,MOJ,MOA,MOA)*FTDM(MOJ,MOI)-2.0D0*TALL(MOI,MOA,MOA,MOJ)*FTDM(MOJ,MOI) &
       +4.0D0*TALL(MOJ,MOI,MOA,MOA)*FTDM(MOI,MOJ)-2.0D0*TALL(MOJ,MOA,MOA,MOI)*FTDM(MOI,MOJ)
    TEMPORARY=TEMPORARY*FI*(1.0D0-FJ)*FA*(1.0D0-FA)
    IF(DABS(D)>1.0D-10) THEN
       ! WRITE(*,*) 'MOI=',MOI,' MOJ=',MOJ,' MOA=',MOA,' TEMPORARY=',TEMPORARY/D
       MP2_MU=MP2_MU+TEMPORARY/D
       MP2_U=MP2_U-TEMPORARY*BETA*EA/D
    ELSE
       ! WRITE(*,*) 'MOI=',MOI,' MOJ=',MOJ,' MOA=',MOA,' TEMPORARY=',TEMPORARY
       MP2_MU=MP2_MU-TEMPORARY*BETA/2.0D0
       MP2_U=MP2_U+TEMPORARY*BETA*BETA*EA/2.0D0
    ENDIF
     DO MOB=ICORE+1,IALL(0,0,0)-IVIRTCORE
      FB=1.0D0/(DEXP((EPSILON(MOB,0,0,0)-FERMI)*BETA)+1.0D0)
      EB=EPSILON(MOB,0,0,0)
      D=EI+EJ-EA-EB
      TEMPORARY=(2.0D0*TALL(MOI,MOA,MOJ,MOB)*DCONJG(TALL(MOI,MOA,MOJ,MOB)) &
         -DREAL(TALL(MOI,MOA,MOJ,MOB)*DCONJG(TALL(MOJ,MOA,MOI,MOB))))*FI*FJ*(1.0D0-FA)*(1.0D0-FB)
      ! TEMPORARY=DREAL(TALL(MOI,MOA,MOJ,MOB)*DCONJG(TALL(MOI,MOA,MOJ,MOB))) &
      !    +DREAL(TALL(MOJ,MOA,MOI,MOB)*DCONJG(TALL(MOJ,MOA,MOI,MOB))) &
      !    -DREAL(TALL(MOI,MOA,MOJ,MOB)*DCONJG(TALL(MOJ,MOA,MOI,MOB)))

      ! TEMPORARY = TEMPORARY*FI*FJ*(1.0D0-FA)*(1.0D0-FB)
      IF(DABS(D)>1.0D-10) THEN
       MP2_MU=MP2_MU+TEMPORARY*((1.0D0-FI)+(1.0D0-FJ)-FA-FB)/D
       MP2_U=MP2_U-TEMPORARY*(EI*(1.0D0-FI)+EJ*(1.0D0-FJ)-EA*FA-EB*FB)*BETA/D
      ELSE
       MP2_MU=MP2_MU-TEMPORARY*((1.0D0-FI)+(1.0D0-FJ)-FA-FB)*BETA/2.0D0
       MP2_U=MP2_U+TEMPORARY*(EI*(1.0D0-FI)+EJ*(1.0D0-FJ)-EA*FA-EB*FB)*BETA*BETA/2.0D0
      ENDIF
       ! WRITE(6,'(A,I5,2X,I5,2X,I5,2X,I5,2X,F20.10,2X,F20.10)') &
       ! 'DEBUG: MOI,MOJ,MOA,MOB,MP2_MU,MP2_U = ', MOI,MOJ,MOA,MOB,MP2_MU,MP2_U
      ENDDO
     ENDDO
    ENDDO
   ENDDO
   ...
   DO MOI=ICORE+1,IALL(0,0,0)-IVIRTCORE
    FI=1.0D0/(DEXP((EPSILON(MOI,0,0,0)-FERMI)*BETA)+1.0D0)
    EI=EPSILON(MOI,0,0,0)
    MP2_OMEGA=MP2_OMEGA+BETA*MP1_MU*2.0D0*FTDM(MOI,MOI)*FI*(1.0D0-FI)-BETA*MP1_MU*MP1_MU*FI*(1.0D0-FI)
    MP2_U=MP2_U+4.0D0*BETA*MP1_MU*FTDM(MOI,MOI)*FI*(1.0D0-FI)-2.0D0*BETA*MP1_MU*MP1_MU*FI*(1.0D0-FI)
    DO MOJ=ICORE+1,IALL(0,0,0)-IVIRTCORE
     FJ=1.0D0/(DEXP((EPSILON(MOJ,0,0,0)-FERMI)*BETA)+1.0D0)
     EJ=EPSILON(MOJ,0,0,0)
     D=EI-EJ
     TEMPORARY=2.0D0*FTDM(MOI,MOJ)*FTDM(MOI,MOJ)*FI*(1-FJ)
     IF(DABS(D)>1.0D-10) THEN
       MP2_OMEGA=MP2_OMEGA+TEMPORARY/D
       MP2_U=MP2_U+TEMPORARY/D
     ELSE
       MP2_OMEGA=MP2_OMEGA-TEMPORARY*BETA/2.0D0
       MP2_U=MP2_U-TEMPORARY*BETA
     ENDIF
     DO MOA=ICORE+1,IALL(0,0,0)-IVIRTCORE
       FA=1.0D0/(DEXP((EPSILON(MOA,0,0,0)-FERMI)*BETA)+1.0D0)
       EA=EPSILON(MOA,0,0,0)
       DO MOB=ICORE+1,IALL(0,0,0)-IVIRTCORE
        ...
        IF (MOI==IOCC .AND. MOJ<=IOCC .AND. MOA>IOCC .AND. MOB>IOCC) THEN
           IF (ABS(D) > 1.0D-10) THEN
           ! ! DEBUG TIME!
           ! WRITE(6,'(A, F20.10)') '(ABS(TALL(MOI,MOA,MOJ,MOB))**2 = ', ABS(TALL(MOI,MOA,MOJ,MOB))**2
           ! WRITE(6,'(A, F20.10)') '(ABS(TALL(MOJ,MOA,MOI,MOB))**2 = ', ABS(TALL(MOJ,MOA,MOI,MOB))**2
           SIGMA_H=SIGMA_H+2.0D0*(ABS(TALL(MOI,MOA,MOJ,MOB))**2 &
              +ABS(TALL(MOJ,MOA,MOI,MOB))**2 &
              -DREAL(TALL(MOI,MOA,MOJ,MOB)*DCONJG(TALL(MOJ,MOA,MOI,MOB))))/D
           ENDIF
        ENDIF
        IF (MOI==IOCC+1 .AND. MOJ<=IOCC .AND. MOA>IOCC .AND. MOB>IOCC) THEN
           IF (ABS(D) > 1.0D-10) THEN
           SIGMA_L=SIGMA_L+2.0D0*(ABS(TALL(MOI,MOA,MOJ,MOB))**2 &
              +ABS(TALL(MOJ,MOA,MOI,MOB))**2 &
              -DREAL(TALL(MOI,MOA,MOJ,MOB)*DCONJG(TALL(MOJ,MOA,MOI,MOB))))/D
           ENDIF
        ENDIF
        IF (MOI<=IOCC .AND. MOJ<=IOCC .AND. MOA==IOCC .AND. MOB>IOCC) THEN
           IF(ABS(D) > 1.0D-10) THEN
           SIGMA_H=SIGMA_H-2.0D0*(ABS(TALL(MOI,MOA,MOJ,MOB)))**2 &
              +ABS(TALL(MOI,MOB,MOJ,MOA))**2 &
              -DREAL(TALL(MOI,MOB,MOJ,MOA)*DCONJG(TALL(MOI,MOA,MOJ,MOB)))/D
           ENDIF
        ENDIF 
        IF (MOI<=IOCC .AND. MOJ<=IOCC .AND. MOA==IOCC+1 .AND. MOB>IOCC) THEN
           IF(ABS(D) > 1.0D-10) THEN
           SIGMA_L=SIGMA_L-2.0D0*(ABS(TALL(MOI,MOA,MOJ,MOB)))**2 &
              +ABS(TALL(MOI,MOB,MOJ,MOA))**2 &
              -DREAL(TALL(MOI,MOB,MOJ,MOA)*DCONJG(TALL(MOI,MOA,MOJ,MOB)))/D
   ...
   ! WRITE(6,'(A,F20.10,A)') 'DEBUG ERRORTRAP VALUE FOR MP2_U = ',ERRORTRAP,' HARTREE'
   IF (DABS(NORMDIRAC) > ERRORTRAP) THEN
    MP2_MU=MP2_MU/NORMDIRAC
   ELSE
    WRITE(6,'(A)') 'NORMDIRAC IS TOO SMALL. SETTING MP2_MU TO LIMITING VALUE' 
    WRITE(6,'(A)') 'Physical Review A 103, no. 1 (2021): 012223. EQUATION 40'
    MP2_MU=(SIGMA_H+SIGMA_L)/2.0D0
   ENDIF

   DO MOI=ICORE+1,IALL(0,0,0)-IVIRTCORE
    FI=1.0D0/(DEXP((EPSILON(MOI,0,0,0)-FERMI)*BETA)+1.0D0)
    EI=EPSILON(MOI,0,0,0)
    MP2_U=MP2_U+BETA*MP2_MU*2.0D0*EI*FI*(1.0D0-FI)     
   ENDDO


   MP2_OMEGA=MP2_OMEGA-MP2_MU*NBAR

   IF (MYID == 0) THEN
    IF(DABS(NORMDIRAC) < ERRORTRAP) THEN
    WRITE(6,'(A)') 'CHEMICAL POTENTIAL HAS BEEN CORRECTED USING LIMITING CASE FOR LOW TEMPERATURE'
    WRITE(6, '(A,F20.15)') 'SIGMA_H = ',SIGMA_H
    WRITE(6, '(A,F20.15)') 'SIGMA_L = ',SIGMA_L
    ENDIF
   
    WRITE(6,'(A,F20.15,A)') 'MP0_MU    = ',MP0_MU,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'MP0_OMEGA = ',MP0_OMEGA,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'MP0_U     = ',MP0_U,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'MP1 MU    = ',MP1_MU,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'MP1 OMEGA = ',MP1_OMEGA,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'MP1 U     = ',MP1_U,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'MP2 MU    = ',MP2_MU,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'MP2 OMEGA = ',MP2_OMEGA,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'MP2 U     = ',MP2_U,' HARTREE'
   ENDIF
...
!    WRITE(6,'(A,F20.6,A)') 'MP2_R2    = ',MP2_R2,' HARTREE'
   ...
   ! DEALLOCATE(TNNNKK,TALL,KL)
   DEALLOCATE(TOK,TVOKK,TOVOKK)


   ELSE

   WRITE(6, '(A)') 'ENTERING SOLIDS BRANCH'
...
!  ALLOCATE(KVC3(ICORE+1:IALLMAX-IVIRTCORE),LKVC(ICORE+1:IALLMAX-IVIRTCORE,-KVC:KVC))
   ...
   ! PRINT EPSILON AND FTDS FOR DEBUGGING
   ! PRINT EPSILON
   IF ((IOPTN(9) >= 2).AND.(MYID == 0)) THEN
   WRITE(6,'(A)') 'ORBITAL ENERGIES FOR ALL K-POINTS:'
   DO KBX=-KVCX2,MAX(0,KVCX2-1)
   DO KBY=-KVCY2,MAX(0,KVCY2-1)
   DO KBZ=-KVCZ2,MAX(0,KVCZ2-1)
    WRITE(6,'(/,A,I4,A,I4,A,I4,A)') 'EPSILON AT K-POINT (', KBX, ',', KBY, ',', KBZ, '):'
    DO MOI=ICORE+1,IALL(KBX,KBY,KBZ)-IVIRTCORE
     WRITE(6,'(A,I4,A,F12.6)') ' MOI =', MOI, ': ', EPSILON(MOI,KBX,KBY,KBZ)
    ENDDO
   ENDDO
   ENDDO
   ENDDO
   ...
   IF ((IOPTN(9) >= 2).AND.(MYID == 0)) THEN
   WRITE(6,'(A)') 'FOCK MATRIX AT MP TEMPERATURE FOR ALL K-POINTS:'
   DO KBX = -KVCX2, MAX(0, KVCX2 - 1)
    WRITE(6,'(/,A,I4,A)') 'FOCK MATRIX AT MP TEMPERATURE AT K-POINT (', KBX * NMOD, ',0,0):'
    DO MOI = ICORE + 1, IALL(KBX,0,0) - IVIRTCORE
     DO MOJ = ICORE + 1, IALL(KBX,0,0) - IVIRTCORE
      WRITE(6,'(2F12.6,2X)', ADVANCE='NO') REAL(FTDS(MOI,MOJ,KBX,0,0)), AIMAG(FTDS(MOI,MOJ,KBX,0,0))
     ENDDO
     WRITE(6,*)  ! line break at end of row
    ENDDO
   ENDDO
   ENDIF
   ...
   ! PRINTING SIZE OF ARRAYS FOR DEBUGGING

   IF(IOPTN(9)==3) THEN
      WRITE(*,*) 'PHASEX: ', SHAPE(PHASEX)
      WRITE(*,*) 'PHASEY: ', SHAPE(PHASEY)
      WRITE(*,*) 'PHASEZ: ', SHAPE(PHASEZ)
   ENDIF

   ! ALLOCATE(QEPSILON_TMP(NCGS,-KVCX:KVCX,-KVCY:KVCY,-KVCZ:KVCZ))
   ! ALLOCATE(FTDS(NCGS,NCGS,-KVCX2:KVCX2,-KVCY2:KVCY2,-KVCZ2:KVCZ2))

   IF (MYID == 0) WRITE(6,'(A)') 'CALCULATE MP2 ENERGY (ALGORITHM 1) FOR SOLIDS AT FINITE TEMPERATURE'
   IF (IOPTN(27) /= 0) CALL WARNING('FROZEN CORE APPROXIMATION WILL BE USED')
   IF (IOPTN(55) /= 0) CALL WARNING('FROZEN VIRTUAL APPROXIMATION WILL BE USED')
   IF (IALLMAX-IOCC-IVIRTCORE <= 0) THEN
    CALL WARNING('THERE IS NO VIRTUAL ORBITAL --- MP2 CORRECTION IS NULL')
...
!  ENDIF
   ...
   CALL PFLUSH(6)
   ALLOCATE(TNNNKK(NCGS,NCGS,NCGS,(2*CEL1X+1)*(2*CEL1Y+1)*(2*CEL1Z+1),(2*CEL2X+1)*(2*CEL2Y+1)*(2*CEL2Z+1)))
   IF (MYID == 0) WRITE(6,'(F20.0,A)') DFLOAT((IALLMAX-IVIRTCORE-ICORE-2)*(2*KVCX2+1)*(2*KVCY2+1)*(2*KVCZ2+1)) &
    /1.0D3,' K WORDS (TOK)'
   CALL PFLUSH(6)
   ALLOCATE(TOK(ICORE+1:IALLMAX-IVIRTCORE,MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2)))
   IF (MYID == 0) WRITE(6,'(F20.0,A)') DFLOAT((IALLMAX-IVIRTCORE-ICORE) &
    *(IALLMAX-IVIRTCORE-ICORE-2)*MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2) &
    *MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2))/1.0D3,' K WORDS (TVOKK)'
   CALL PFLUSH(6)
   ALLOCATE(TVOKK(ICORE+1:IALLMAX-IVIRTCORE,ICORE+1:IALLMAX-IVIRTCORE,&
                  MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2),MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2)))
   IF (MYID == 0) WRITE(6,'(F20.0,A)') DFLOAT(IALLMAX-IVIRTCORE-ICORE) &
                                      *DFLOAT(IALLMAX-IVIRTCORE-ICORE-2)**2 &
                                      *DFLOAT(MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2) &
                                             *MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2))/1.0D3,' K WORDS (TOVOKK)'
   CALL PFLUSH(6)
   ALLOCATE(TOVOKK(ICORE+1:IALLMAX-IVIRTCORE,ICORE+1:IALLMAX-IVIRTCORE, &
                   ICORE+1:IALLMAX-IVIRTCORE, &
                   MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2),MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2)))

   ! PRINTING SIZE OF EACH MATRIX FOR DEBUGGING

   IF(IOPTN(9)==3) THEN
      WRITE(*,*) 'TNNNKK: ', SHAPE(TNNNKK)
      WRITE(*,*) 'TOK: ', SHAPE(TOK)
      WRITE(*,*) 'TVOKK: ', SHAPE(TVOKK)
      WRITE(*,*) 'TOVOKK: ', SHAPE(TOVOKK)
   ENDIF
   ...
   QEPSILON=0.0D0
   ! QEPSILON_TMP=0.0D0
   ! FTDS=DCMPLX(0.0D0,0.0D0)

   DO KBX=-KVCX2,MAX(0,KVCX2-1)
   DO KBY=-KVCY2,MAX(0,KVCY2-1)
   DO KBZ=-KVCZ2,MAX(0,KVCZ2-1)
   DO MOB=ICORE+1,IALL(KBX,KBY,KBZ)-IVIRTCORE
      FTDS(MOB,MOB,KBX,KBY,KBZ)=FTDS(MOB,MOB,KBX*NMOD,KBY*NMOD,KBZ*NMOD)-EPSILON(MOB,KBX*NMOD,KBY*NMOD,KBZ*NMOD)
   ENDDO
   ENDDO
   ENDDO
   ENDDO

   ! IF(IOPTN(98)==IOPTN(108)) THEN
   !  FTDS=DCMPLX(0.0D0,0.0D0)
   ! ENDIF


   ! PRINTING FTDS MATRIX ELEMENTS FOR DEBUGGING PER K POINT
   IF ((IOPTN(9) >= 1).AND.(MYID == 0)) THEN
    DO KBX = -KVCX2, MAX(0, KVCX2 - 1)
     WRITE(6,'(/,A,I4,A)') 'FTDS MATRIX AT K-POINT (', KBX * NMOD, ',0,0):'
     DO MOI = ICORE + 1, IALL(KBX,0,0) - IVIRTCORE
      DO MOJ = ICORE + 1, IALL(KBX,0,0) - IVIRTCORE
       WRITE(6,'(2F20.12,2X)', ADVANCE='NO') REAL(FTDS(MOI,MOJ,KBX,0,0)), AIMAG(FTDS(MOI,MOJ,KBX,0,0))
      ENDDO
      WRITE(6,*)  ! line break at end of row
     ENDDO
    ENDDO
   ...
   NKPTOTAL=DFLOAT(MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2))
   WRITE(6,'(A,F10.6,A)') 'TOTAL NUMBER OF K-POINTS = ',NKPTOTAL,''

   ! PRINTING NUMBER OF ORBITALS AT EACH K-POINT
   ! DO KBX=-KVCX2,MAX(0,KVCX2-1)
   ! DO KBY=-KVCY2,MAX(0,KVCY2-1)
   ! DO KBZ=-KVCZ2,MAX(0,KVCZ2-1)
   !    WRITE(6,'(A, 3I4, A, I4)') 'NUMBER OF ORBITALS AT K-POINT (',KBX*NMOD,KBY*NMOD,KBZ*NMOD,') = ', &
   !     IALL(KBX,KBY,KBZ)-IVIRTCORE
   ! ENDDO
   ! ENDDO
   ! ENDDO
   ...
   DO KBZ=-KVCZ2,MAX(0,KVCZ2-1)
    DO MOI=ICORE+1,IALL(KBX,KBY,KBZ)-IVIRTCORE
     EB=EPSILON(MOI,KBX*NMOD,KBY*NMOD,KBZ*NMOD)
     NBAR=NBAR+2.0D0*(1.0D0/(DEXP((EB-MU)*BETA)+1.0D0))
    ENDDO
   ENDDO
   ENDDO
   ENDDO

   NBAR=NBAR*1.0D0/DFLOAT(MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2))
   TEMPORARY=0.0D0
   ERRORTRAP=1.0D-15


   WRITE(6,'(A,F20.6,A)') 'NBAR PER UNIT CELL = ',NBAR,' ELECTRONS'
   ...
   DO MOI=ICORE+1,IALL(KBX,KBY,KBZ)-IVIRTCORE
    EB=EPSILON(MOI,KBX*NMOD,KBY*NMOD,KBZ*NMOD)
    FB=1.0D0/(DEXP((EB-MU)*BETA)+1.0D0)
    NORMDIRAC=NORMDIRAC+2.0D0*FB*(1.0D0-FB)
   ENDDO
   ENDDO
   ENDDO
   ENDDO

   WRITE(6,'(A,F20.15,A)') 'NORMDIRAC BEFORE NORMALIZATION BY K-POINTS= ',NORMDIRAC,' '
   ...
   DO MOI=ICORE+1,IALL(KBX,KBY,KBZ)-IVIRTCORE
    EB=EPSILON(MOI,KBX*NMOD,KBY*NMOD,KBZ*NMOD)
    FB=1.0D0/(DEXP((EB-MU)*BETA)+1.0D0)
    MP0_OMEGA=MP0_OMEGA+2.0D0*(EB-MU)+2.0D0*DLOG(FB)/BETA
    IF (FB > 1.0D-10) THEN
     MP0_OMEGA1=MP0_OMEGA1+2.0D0*(EB-MU)+2.0D0*DLOG(FB)/BETA
    ENDIF
    MP0_U=MP0_U+2.0D0*EB*FB
   ENDDO
   ENDDO
   ENDDO
   ENDDO
   ...
   TEMPORARY=0.0D0
   ! MP0_OMEGA = MP0_OMEGA/NKPTOTAL
   ! MP0_U = MP0_U/NKPTOTAL

   MP0_OMEGA=MP0_OMEGA/NKPTOTAL+NUCLEAR_REPULSION
   MP0_OMEGA1=MP0_OMEGA1/NKPTOTAL+NUCLEAR_REPULSION
   MP0_U=MP0_U/NKPTOTAL+NUCLEAR_REPULSION


   ! NORMDIRAC=NORMDIRAC/NKPTOTAL

   MP1_MU=0.0D0
   MP1_OMEGA=0.0D0
   MP1_U=0.0D0
   MP1_OMEGA_1E=0.0D0
   MP1_OMEGA_1E_TMP=0.0D0
   MP1_U_1E=0.0D0
   MP1_U_1E_TMP=0.0D0
   ...
   MP1_U_2E_TMP=0.0D0
   MP1_OMEGA_2E=0.0D0
   MP1_OMEGA_2E_TMP=0.0D0  

   MP2_MU_1E=0.0D0
   MP2_MU_1E_TMP=0.0D0
   MP2_OMEGA_1E=0.0D0
   MP2_OMEGA_1E_TMP=0.0D0
   MP2_OMEGA_2E=0.0D0
   MP2_OMEGA_2E_TMP=0.0D0
   ...
   DO KIX=-KVCX2,MAX(0,KVCX2-1)
   DO KIY=-KVCY2,MAX(0,KVCY2-1)
   DO KIZ=-KVCZ2,MAX(0,KVCZ2-1)
   DO MOI=ICORE+1,IALL(KIX,KIY,KIZ)-IVIRTCORE
    EI=EPSILON(MOI,KIX*NMOD,KIY*NMOD,KIZ*NMOD)
    FI=1.0D0/(EXP((EI-MU)*BETA)+1.0D0)
    TEMPORARY=1.0D0*DREAL(FTDS(MOI,MOI,KIX*NMOD,KIY*NMOD,KIZ*NMOD))*FI
    MP1_OMEGA_1E_TMP=MP1_OMEGA_1E_TMP+2.0D0*TEMPORARY
    MP1_U_1E_TMP=MP1_U_1E_TMP+2.0D0*TEMPORARY
    MP1_MU=MP1_MU+2.0D0*TEMPORARY*(1.0D0-FI)
    ! WRITE(*,*) 'DEBUG MP1_MU CONTRIBUTION: ', MP1_MU
   ENDDO
   ENDDO
   ENDDO
   ENDDO
   ...
   ! Normalizing after calulating MU(1) and then sum is normalized with K points.
   NORMDIRAC=NORMDIRAC/NKPTOTAL


   MP1_OMEGA_1E=MP1_OMEGA_1E+MP1_OMEGA_1E_TMP/NKPTOTAL
   MP1_OMEGA_1E=MP1_OMEGA_1E-MP1_MU*NBAR

   MP1_U_1E=MP1_U_1E+MP1_U_1E_TMP/NKPTOTAL
   ...
   DO KIX=-KVCX2,MAX(0,KVCX2-1)
    DO KIY=-KVCY2,MAX(0,KVCY2-1)
     DO KIZ=-KVCZ2,MAX(0,KVCZ2-1)
      DO MOI=ICORE+1,IALL(KIX,KIY,KIZ)-IVIRTCORE
       EI=EPSILON(MOI,KIX*NMOD,KIY*NMOD,KIZ*NMOD)
       FI=1.0D0/(DEXP((EI-MU)*BETA)+1.0D0)
       TEMPORARY=BETA*2.0D0*(DREAL(FTDS(MOI,MOI,KIX*NMOD,KIY*NMOD,KIZ*NMOD))-MP1_MU)*EI*FI*(1.0D0-FI)
       ! WRITE(*,*) 'DEBUG TEMPORARY CONTRIBUTION: ', TEMPORARY
       MP1_U_1E_TMP=MP1_U_1E_TMP-TEMPORARY
       MP2_OMEGA_1E_TMP=MP2_OMEGA_1E_TMP+2.0D0*BETA*MP1_MU & 
          *DREAL(FTDS(MOI,MOI,KIX*NMOD,KIY*NMOD,KIZ*NMOD))*FI*(1.0D0-FI)
       MP2_OMEGA_1E_TMP=MP2_OMEGA_1E_TMP-BETA*MP1_MU*MP1_MU*FI*(1.0D0-FI)
   
       MP2_U_1E_TMP=MP2_U_1E_TMP+2.0D0*2.0D0*BETA*MP1_MU & 
          *DREAL(FTDS(MOI,MOI,KIX*NMOD,KIY*NMOD,KIZ*NMOD))*FI*(1.0D0-FI)
       MP2_U_1E_TMP=MP2_U_1E_TMP-2.0D0*BETA*MP1_MU*MP1_MU*FI*(1.0D0-FI)
   
       MP2_MU_1E_TMP=MP2_MU_1E_TMP+2.0D0*BETA*MP1_MU & 
          *DREAL(FTDS(MOI,MOI,KIX*NMOD,KIY*NMOD,KIZ*NMOD))*FI*(1.0D0-FI)*((1.0D0-FI)-FI)
       MP2_MU_1E_TMP=MP2_MU_1E_TMP-BETA*MP1_MU*MP1_MU*FI*(1.0D0-FI)*((1.0D0-FI)-FI)
   
       MP2_U_1E_TMP=MP2_U_1E_TMP-2.0D0*BETA*BETA*MP1_MU & 
          *DREAL(FTDS(MOI,MOI,KIX*NMOD,KIY*NMOD,KIZ*NMOD))*FI*(1.0D0-FI)*((1.0D0-FI)-FI)*EI
       MP2_U_1E_TMP=MP2_U_1E_TMP+BETA*BETA*MP1_MU*MP1_MU*FI*(1.0D0-FI)*((1.0D0-FI)-FI)*EI
   
       DO MOJ=ICORE+1,IALL(KIX,KIY,KIZ)-IVIRTCORE
        EJ=EPSILON(MOJ,KIX*NMOD,KIY*NMOD,KIZ*NMOD)
        FJ=1.0D0/(DEXP((EJ-MU)*BETA)+1.0D0)
        D = EI-EJ
        IF (DABS(D) > 1.0D-12) THEN
      !   IF (DABS(D) .NE. 0.0D0) THEN
         TEMPORARY=1.0D0*FTDS(MOI,MOJ,KIX*NMOD,KIY*NMOD,KIZ*NMOD)*DCONJG(FTDS(MOI,MOJ,KIX*NMOD,KIY*NMOD,KIZ*NMOD))&
          *FI*(1-FJ)/D
         MP2_OMEGA_2E_TMP=MP2_OMEGA_2E_TMP+2.0D0*TEMPORARY
         MP2_U_2E_TMP=MP2_U_2E_TMP+2.0D0*TEMPORARY
         TEMPORARY=1.0D0*FTDS(MOI,MOJ,KIX*NMOD,KIY*NMOD,KIZ*NMOD)*DCONJG(FTDS(MOI,MOJ,KIX*NMOD,KIY*NMOD,KIZ*NMOD))&
          *FI*(1-FJ)*((1-FI)-FJ)/D
         MP2_MU_2E_TMP=MP2_MU_2E_TMP+2.0D0*TEMPORARY
         TEMPORARY=1.0D0*FTDS(MOI,MOJ,KIX*NMOD,KIY*NMOD,KIZ*NMOD)*DCONJG(FTDS(MOI,MOJ,KIX*NMOD,KIY*NMOD,KIZ*NMOD))&
          *FI*(1-FJ)*((1-FI)*EI-FJ*EJ)/D
         MP2_U_2E_TMP=MP2_U_2E_TMP-2.0D0*BETA*TEMPORARY
        ELSE
         TEMPORARY=1.0D0*FTDS(MOI,MOJ,KIX*NMOD,KIY*NMOD,KIZ*NMOD)*DCONJG(FTDS(MOI,MOJ,KIX*NMOD,KIY*NMOD,KIZ*NMOD))&
          *FI*(1-FJ)
         MP2_OMEGA_2E_TMP=MP2_OMEGA_2E_TMP-BETA*TEMPORARY
         MP2_U_2E_TMP=MP2_U_2E_TMP-2.0D0*BETA*TEMPORARY
         TEMPORARY=1.0D0*FTDS(MOI,MOJ,KIX*NMOD,KIY*NMOD,KIZ*NMOD)*DCONJG(FTDS(MOI,MOJ,KIX*NMOD,KIY*NMOD,KIZ*NMOD))&
          *FI*(1-FJ)*((1-FI)-FJ)
         MP2_MU_2E_TMP=MP2_MU_2E_TMP-BETA*TEMPORARY
         TEMPORARY=1.0D0*FTDS(MOI,MOJ,KIX*NMOD,KIY*NMOD,KIZ*NMOD)*DCONJG(FTDS(MOI,MOJ,KIX*NMOD,KIY*NMOD,KIZ*NMOD))&
          *FI*(1-FJ)*((1-FI)*EI-FJ*EJ)
         MP2_U_2E_TMP=MP2_U_2E_TMP+BETA*BETA*TEMPORARY
        ENDIF
       ENDDO
      ENDDO
    ENDDO
   ...
   ! WRITE(*,*) 'MP1_U CONTRIBUTION FROM 1E TERMS: ', MP1_U_1E


   MP2_OMEGA_1E=MP2_OMEGA_1E+MP2_OMEGA_1E_TMP/NKPTOTAL
   MP2_U_1E=MP2_U_1E+MP2_U_1E_TMP/NKPTOTAL
   ! WRITE(*,*) 'MP2_U CONTRIBUTION FROM 1E TERMS: ', MP2_U_1E
   MP2_MU_1E=MP2_MU_1E+MP2_MU_1E_TMP/NKPTOTAL

   MP2_OMEGA_2E=MP2_OMEGA_2E+MP2_OMEGA_2E_TMP/NKPTOTAL
   MP2_MU_2E=MP2_MU_2E+MP2_MU_2E_TMP/NKPTOTAL
   MP2_U_2E=MP2_U_2E+MP2_U_2E_TMP/NKPTOTAL
...
!write(*,*) 'Process:',myid,' reading from ',TRIM(COPTN(1))//'.ao.'//FILEAPPENDIX(2:6)
     ...
     DO
      ...
      ! WRITE(*,*) 'indices: ',I,J,K,L,' Q1: (',Q1X,Q1Y,Q1Z,') Q2: (',Q2X,Q2Y,Q2Z,') Q3: (',Q3X,Q3Y,Q3Z,')'
     ...
     TOVOKK(ICORE+1:IALLMAX-IVIRTCORE,ICORE+1:IALLMAX-IVIRTCORE, &
            ...
            1:MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2),1:MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2))=DCMPLX(0.0D0,0.0D0)
     DO I=1,NCGS
      TVOKK(ICORE+1:IALLMAX-IVIRTCORE,ICORE+1:IALLMAX-IVIRTCORE, &
      1:MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2),1:MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2))=DCMPLX(0.0D0,0.0D0)
      DO Q1X=-CEL1X,CEL1X
      DO Q1Y=-CEL1Y,CEL1Y
      DO Q1Z=-CEL1Z,CEL1Z
       DO J=1,NCGS
        TOK(ICORE+1:IALLMAX-IVIRTCORE,1:MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2))=DCMPLX(0.0D0,0.0D0)
        DO Q3X=-CEL2X,CEL2X
        DO Q3Y=-CEL2Y,CEL2Y
        DO Q3Z=-CEL2Z,CEL2Z
         DO K=1,NCGS
          ...
          DO KJY=-KVCY2,MAX(0,KVCY2-1)
          DO KJZ=-KVCZ2,MAX(0,KVCZ2-1)
           DO MOJ=ICORE+1,IALL(KJX,KJY,KJZ)-IVIRTCORE
            IF (HELIX == 0.0D0) THEN
             TOK(MOJ,((KJX+KVCX2)*MAX(1,2*KVCY2)+(KJY+KVCY2))*MAX(1,2*KVCZ2)+KJZ+KVCZ2+1)= &
             TOK(MOJ,((KJX+KVCX2)*MAX(1,2*KVCY2)+(KJY+KVCY2))*MAX(1,2*KVCZ2)+KJZ+KVCZ2+1)+ &
             TNNNKK(I,J,K,((Q1X+CEL1X)*(2*CEL1Y+1)+(Q1Y+CEL1Y))*(2*CEL1Z+1)+(Q1Z+CEL1Z)+1, &
        ...
        DO KJZ=-KVCZ2,MAX(0,KVCZ2-1)
         ...
         DO KAZ=-KVCZ2,MAX(0,KVCZ2-1)
          DO MOJ=ICORE+1,IALL(KJX,KJY,KJZ)-IVIRTCORE
           DO MOA=ICORE+1,IALL(KAX,KAY,KAZ)-IVIRTCORE
            IF (HELIX == 0.0D0) THEN
             TVOKK(MOA,MOJ,((KAX+KVCX2)*MAX(1,2*KVCY2)+(KAY+KVCY2))*MAX(1,2*KVCZ2)+KAZ+KVCZ2+1, &
                           ((KJX+KVCX2)*MAX(1,2*KVCY2)+(KJY+KVCY2))*MAX(1,2*KVCZ2)+KJZ+KVCZ2+1)= &
             TVOKK(MOA,MOJ,((KAX+KVCX2)*MAX(1,2*KVCY2)+(KAY+KVCY2))*MAX(1,2*KVCZ2)+KAZ+KVCZ2+1, &
     ...
     DO KJZ=-KVCZ2,MAX(0,KVCZ2-1)
     DO MOJ=ICORE+1,IALL(KJX,KJY,KJZ)-IVIRTCORE
       EB=EPSILON(MOB,KBX*NMOD,KBY*NMOD,KBZ*NMOD)
       EJ=EPSILON(MOJ,KJX*NMOD,KJY*NMOD,KJZ*NMOD)
       FB=1.0D0/(DEXP((EB-MU)*BETA)+1.0D0)
       FJ=1.0D0/(DEXP((EJ-MU)*BETA)+1.0D0)

       LINIDXJ = ((KJX+KVCX2)*MAX(1,2*KVCY2)+(KJY+KVCY2))*MAX(1,2*KVCZ2)+KJZ+KVCZ2+1
       LINIDXB = ((KBX+KVCX2)*MAX(1,2*KVCY2)+(KBY+KVCY2))*MAX(1,2*KVCZ2)+KBZ+KVCZ2+1

       ! WRITE(*,*) 'LINEARIZED INDEXES: LINIDXJ=',LINIDXJ,' LINIDXB=',LINIDXB

       ! PRINTING REAL AND IMAGINARY PARTS OF TOVOKK
       ! WRITE(*,*) 'TOVOKK VALUES FOR MOJ=',MOJ,' MOB=',MOB,' KB=',KBX,KBY,KBZ,' KJ=',KJX,KJY,KJZ
       ! WRITE(*,*) ' TOVOKK(MOJ,MOJ,MOB,LINIDXJ,LINIDXB)=',TOVOKK(MOJ,MOJ,MOB,LINIDXJ,LINIDXB)
       ! WRITE(*,*) ' DCONJG(TOVOKK(MOB,MOJ,MOJ,LINIDXJ,LINIDXJ))=',DCONJG(TOVOKK(MOB,MOJ,MOJ,LINIDXJ,LINIDXJ))

       TEMPORARY=4.0D0*DREAL(TOVOKK(MOJ,MOJ,MOB,LINIDXJ,LINIDXB))-2.0D0*DCONJG(TOVOKK(MOB,MOJ,MOJ,LINIDXJ,LINIDXJ))
       ! WRITE(*,*) 'MOJ=',MOJ,' MOB=',MOB,' KB=',KBX,KBY,KBZ,' KJ=',KJX,KJY,KJZ,' TEMPORARY=',TEMPORARY, &
       ! ' FB=',FB,' FJ=',FJ
       MP1_OMEGA_2E_TMP=MP1_OMEGA_2E_TMP-0.5D0*TEMPORARY*FB*FJ
       MP1_U_2E_TMP=MP1_U_2E_TMP-0.5D0*TEMPORARY*FB*FJ
       ! WRITE(*,*) 'MP1_U_2E_TMP=',MP1_U_2E_TMP
       MP2_MU_2E_TMP1=MP2_MU_2E_TMP1+BETA*MP1_MU*TEMPORARY*FJ*(1.0D0-FJ)*FB*(1.0D0-FB)
       MP2_U_2E_TMP1=MP2_U_2E_TMP1-BETA*BETA*MP1_MU*TEMPORARY*FJ*(1.0D0-FJ)*EB*FB*(1.0D0-FB)
      ...
      DO KAZ=-KVCZ2,MAX(0,KVCZ2-1)
      DO MOA=ICORE+1,IALL(KAX,KAY,KAZ)-IVIRTCORE
       EA=EPSILON(MOA,KAX*NMOD,KAY*NMOD,KAZ*NMOD)
       FA=1.0D0/(DEXP((EA-MU)*BETA)+1.0D0)
       LINIDXA = ((KAX+KVCX2)*MAX(1,2*KVCY2)+(KAY+KVCY2))*MAX(1,2*KVCZ2)+KAZ+KVCZ2+1

       IF(KAX==KJX .AND. KAY==KJY .AND. KAZ==KJZ) THEN
         D=EJ-EA

         ! WRITE(*,*) 'MOA=',MOA,' MOJ=',MOJ,' MOB=',MOB,' KB=',KBX,KBY,KBZ,' KJ=',KJX,KJY,KJZ,' TOVOKK VALUES: ', &
         ! TOVOKK(MOJ,MOA,MOB,LINIDXA,LINIDXB), ' ', TOVOKK(MOB,MOA,MOJ,LINIDXA,LINIDXJ), & 
         ! TOVOKK(MOA,MOJ,MOB,LINIDXJ,LINIDXB), ' ', TOVOKK(MOB,MOJ,MOA,LINIDXJ,LINIDXA)
         IF(DABS(D) > 1.0D-12) THEN
         ! IF(DABS(D) .NE. 0.0D0) THEN
          TEMPORARY=DREAL(FTDS(MOA,MOJ,KJX*NMOD,KJY*NMOD,KJZ*NMOD)) &
          *(4.0D0*TOVOKK(MOJ,MOA,MOB,LINIDXA,LINIDXB)-2.0D0*TOVOKK(MOB,MOA,MOJ,LINIDXA,LINIDXJ)) &
          +DREAL(FTDS(MOJ,MOA,KJX*NMOD,KJY*NMOD,KJZ*NMOD)) &
          *(4.0D0*TOVOKK(MOA,MOJ,MOB,LINIDXJ,LINIDXB)-2.0D0*TOVOKK(MOB,MOJ,MOA,LINIDXJ,LINIDXA))
          TEMPORARY=TEMPORARY*FJ*(1.0D0-FA)*FB*(1.0D0-FB)/D

          ! WRITE(*,*) 'MOA=',MOA,' MOJ=',MOJ,' MOB=',MOB,' KB=',KBX,KBY,KBZ,' KJ=',KJX,KJY,KJZ,' TEMPORARY=',TEMPORARY

          MP2_MU_3E_TMP=MP2_MU_3E_TMP+TEMPORARY
          MP2_U_3E_TMP=MP2_U_3E_TMP-BETA*EB*TEMPORARY
         ELSE
          TEMPORARY=DREAL(FTDS(MOA,MOJ,KJX*NMOD,KJY*NMOD,KJZ*NMOD)) &
          *(4.0D0*TOVOKK(MOJ,MOA,MOB,LINIDXA,LINIDXB)-2.0D0*TOVOKK(MOB,MOA,MOJ,LINIDXA,LINIDXJ)) &
          +DREAL(FTDS(MOJ,MOA,KJX*NMOD,KJY*NMOD,KJZ*NMOD)) &
          *(4.0D0*TOVOKK(MOA,MOJ,MOB,LINIDXJ,LINIDXB)-2.0D0*TOVOKK(MOB,MOJ,MOA,LINIDXJ,LINIDXA))
          TEMPORARY=TEMPORARY*FJ*(1.0D0-FA)*FB*(1.0D0-FB)

          ! WRITE(*,*) 'MOA=',MOA,' MOJ=',MOJ,' MOB=',MOB,' KB=',KBX,KBY,KBZ,' KJ=',KJX,KJY,KJZ,' TEMPORARY=',TEMPORARY


          MP2_MU_3E_TMP=MP2_MU_3E_TMP-0.5D0*BETA*TEMPORARY
          MP2_U_3E_TMP=MP2_U_3E_TMP+0.5D0*BETA*BETA*EB*TEMPORARY
         ENDIF
       ENDIF
       ...
       IF (KIZ < -KVCZ2) KIZ=KIZ+2*KVCZ2
         DO MOI=ICORE+1,IALL(KIX,KIY,KIZ)-IVIRTCORE
          ...
          FI=1.0D0/(DEXP((EI-MU)*BETA)+1.0D0)
          D=EI+EJ-EA-EB  
          LINIDXI = ((KIX+KVCX2)*MAX(1,2*KVCY2)+(KIY+KVCY2))*MAX(1,2*KVCZ2)+KIZ+KVCZ2+1
            
          IF (ABS(D) > 1.0D-12) THEN
         !  IF (DABS(D) .NE. 0.0D0) THEN
           TEMPORARY=(ABS(TOVOKK(MOI,MOA,MOJ,LINIDXA,LINIDXJ))**2 &
              +ABS(TOVOKK(MOJ,MOA,MOI,LINIDXA,LINIDXI))**2 &
              -DREAL(TOVOKK(MOI,MOA,MOJ,LINIDXA,LINIDXJ)*DCONJG(TOVOKK(MOJ,MOA,MOI,LINIDXA,LINIDXI)))) &
              *FI*FJ*(1.0D0-FA)*(1.0D0-FB)/D
           MP2_OMEGA_4E_TMP=MP2_OMEGA_4E_TMP+TEMPORARY
           MP2_MU_4E_TMP=MP2_MU_4E_TMP+TEMPORARY*((1.0D0-FI)+(1.0D0-FJ)-FA-FB)
           MP2_U_4E_TMP=MP2_U_4E_TMP+TEMPORARY
           MP2_U_4E_TMP=MP2_U_4E_TMP-BETA*TEMPORARY*(EI*(1.0D0-FI)+EJ*(1.0D0-FJ)-EA*FA-EB*FB)
          ELSE
           TEMPORARY=(ABS(TOVOKK(MOI,MOA,MOJ,LINIDXA,LINIDXJ))**2 &
              +ABS(TOVOKK(MOJ,MOA,MOI,LINIDXA,LINIDXI))**2 &
              -DREAL(TOVOKK(MOI,MOA,MOJ,LINIDXA,LINIDXJ)*DCONJG(TOVOKK(MOJ,MOA,MOI,LINIDXA,LINIDXI)))) &
              *FI*FJ*(1.0D0-FA)*(1.0D0-FB)
           MP2_OMEGA_4E_TMP=MP2_OMEGA_4E_TMP-BETA*TEMPORARY/2.0D0
           MP2_MU_4E_TMP=MP2_MU_4E_TMP-BETA*TEMPORARY*((1.0D0-FI)+(1.0D0-FJ)-FA-FB)/2.0D0
           MP2_U_4E_TMP=MP2_U_4E_TMP-BETA*TEMPORARY
           MP2_U_4E_TMP=MP2_U_4E_TMP+BETA*BETA*TEMPORARY*(EI*(1.0D0-FI)+EJ*(1.0D0-FJ)-EA*FA-EB*FB)/2.0D0
          ENDIF

          IF (MOI==IOCC .AND. MOJ<=IOCC .AND. MOA>IOCC .AND. MOB>IOCC) THEN
           IF (ABS(D) > 1.0D-12) THEN
         !   IF (DABS(D) .NE. 0.0D0) THEN
            SIGMA_H=SIGMA_H+2.0D0*(ABS(TOVOKK(MOI,MOA,MOJ,LINIDXA,LINIDXJ))**2 &
              +ABS(TOVOKK(MOJ,MOA,MOI,LINIDXA,LINIDXI))**2 &
              -DREAL(TOVOKK(MOI,MOA,MOJ,LINIDXA,LINIDXJ)*DCONJG(TOVOKK(MOJ,MOA,MOI,LINIDXA,LINIDXI))))/D
           ENDIF
          ENDIF
          IF (MOI==IOCC+1 .AND. MOJ<=IOCC .AND. MOA>IOCC .AND. MOB>IOCC) THEN
           IF (ABS(D) > 1.0D-12) THEN
         !   IF (DABS(D) .NE. 0.0D0) THEN
            SIGMA_L=SIGMA_L+2.0D0*(ABS(TOVOKK(MOI,MOA,MOJ,LINIDXA,LINIDXJ))**2 &
              +ABS(TOVOKK(MOJ,MOA,MOI,LINIDXA,LINIDXI))**2 &
              -DREAL(TOVOKK(MOI,MOA,MOJ,LINIDXA,LINIDXJ)*DCONJG(TOVOKK(MOJ,MOA,MOI,LINIDXA,LINIDXI))))/D
           ENDIF
          ENDIF
         ...
         !   IF (DABS(D) .NE. 0.0D0) THEN
            SIGMA_H=SIGMA_H-2.0D0*(ABS(TOVOKK(MOI,MOA,MOJ,LINIDXA,LINIDXJ)))**2 &
              +ABS(TOVOKK(MOJ,MOA,MOI,LINIDXA,LINIDXI))**2 &
              -DREAL(TOVOKK(MOJ,MOA,MOI,LINIDXA,LINIDXI)*DCONJG(TOVOKK(MOI,MOA,MOJ,LINIDXA,LINIDXJ)))/D
           ENDIF
          ENDIF 
          IF (MOI<=IOCC .AND. MOJ<=IOCC .AND. MOA==IOCC+1 .AND. MOB>IOCC) THEN
           IF(ABS(D) > 1.0D-12) THEN
...
! --- PARALLEL LOOP
     CALL PCPU_TIME(ICPUE)
     IF (MYID == 0) WRITE(6,'(A,F6.2,A,F10.1,A)') ' ... ', &
       DFLOAT((((KBX+KVCX2)*MAX(1,2*KVCY2)+(KBY+KVCY2))*MAX(1,2*KVCZ2)+(KBZ+KVCZ2))*(IALL(KBX,KBY,KBZ)-IVIRTCORE-ICORE) &
       +(MOB-ICORE))/DFLOAT((IALL(KBX,KBY,KBZ)-IVIRTCORE-ICORE)*MAX(1,2*KVCX2)*MAX(1,2*KVCY2)*MAX(1,2*KVCZ2))*100.0D0, &
       ' % OF MP2 ENERGY EVALUATION (CPU / SEC = ',ICPUE-ICPUS,')'
     CALL PCPU_TIME(ICPUS)
     CALL PFLUSH(6)
     ENDIF
...
!write(*,*) emp2s_tmp,emp2t_tmp
   ...
   ! WRITE(*,*) 'MP1_U_2E_TMP BEFORE REDUCE=',MP1_U_2E_TMP
   ! WRITE(*,*) 'MP2_MU_3E_TMP BEFORE REDUCE=',MP2_MU_3E_TMP
   ! WRITE(*,*) 'MP2_U_3E_TMP BEFORE REDUCE=',MP2_U_3E_TMP
   ! WRITE(*,*) 'MP2_U_4E_TMP BEFORE REDUCE=',MP2_U_4E_TMP

   CALL MPI_ALLREDUCE(MP1_OMEGA_2E_TMP,MP1_OMEGA_2E,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MP1_U_2E_TMP,MP1_U_2E,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MP2_MU_2E_TMP1,MP2_MU_2E_TMP,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MP2_U_2E_TMP1,MP2_U_2E_TMP,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MP2_MU_3E_TMP,MP2_MU_3E,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MP2_U_3E_TMP,MP2_U_3E,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MP2_OMEGA_4E_TMP,MP2_OMEGA_4E,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MP2_MU_4E_TMP,MP2_MU_4E,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MP2_U_4E_TMP,MP2_U_4E,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)

   MP1_OMEGA_2E=MP1_OMEGA_2E/NKPTOTAL/NKPTOTAL
   MP1_U_2E=MP1_U_2E/NKPTOTAL/NKPTOTAL
   MP2_MU_2E=MP2_MU_2E+MP2_MU_2E_TMP/NKPTOTAL/NKPTOTAL
   MP2_U_2E=MP2_U_2E+MP2_U_2E_TMP/NKPTOTAL/NKPTOTAL
   MP2_MU_3E=MP2_MU_3E/NKPTOTAL/NKPTOTAL
   MP2_U_3E=MP2_U_3E/NKPTOTAL/NKPTOTAL
   MP2_OMEGA_4E=MP2_OMEGA_4E/NKPTOTAL/NKPTOTAL/NKPTOTAL
   MP2_MU_4E=MP2_MU_4E/NKPTOTAL/NKPTOTAL/NKPTOTAL
   MP2_U_4E=MP2_U_4E/NKPTOTAL/NKPTOTAL/NKPTOTAL

   ! !DEBUG PRINTING OF INDIVIDUAL CONTRIBUTIONS
   ! WRITE(6,'(A,F20.15,A)') 'MP1 OMEGA 2E = ',MP1_OMEGA_2E,' HARTREE'
   ! WRITE(6,'(A,F20.15,A)') 'MP1 U 2E     = ',MP1_U_2E,' HARTREE'
   ! WRITE(6,'(A,F20.15,A)') 'MP2 MU 2E  = ',MP2_MU_2E,' HARTREE'
   ! WRITE(6,'(A,F20.15,A)') 'MP2 U 2E  = ',MP2_U_2E,' HARTREE'
   ! WRITE(6,'(A,F20.15,A)') 'MP2 MU 3E  = ',MP2_MU_3E,' HARTREE'
   ! WRITE(6,'(A,F20.15,A)') 'MP2 U 3E  = ',MP2_U_3E,' HARTREE'
   ! WRITE(6,'(A,F20.15,A)') 'MP2 OMEGA 4E = ',MP2_OMEGA_4E,' HARTREE'
   ! WRITE(6,'(A,F20.15,A)') 'MP2 MU 4E  = ',MP2_MU_4E,' HARTREE'
   ! WRITE(6,'(A,F20.15,A)') 'MP2 U 4E  = ',MP2_U_4E,' HARTREE'  

   MP1_OMEGA=MP1_OMEGA_1E+MP1_OMEGA_2E
   MP1_U=MP1_U_1E+MP1_U_2E

   MP2_OMEGA=MP2_OMEGA_1E+MP2_OMEGA_2E+MP2_OMEGA_4E
   MP2_MU=MP2_MU_1E+MP2_MU_2E+MP2_MU_3E+MP2_MU_4E
   MP2_U=MP2_U_1E+MP2_U_2E+MP2_U_3E+MP2_U_4E
   ...
   ! WRITE(*,*) 'MP2_U BEFORE MU(2) CORRECTION=',MP2_U
   ...
   DO MOI=ICORE+1,IALL(KIX,KIY,KIZ)-IVIRTCORE
    EI=EPSILON(MOI,KIX*NMOD,KIY*NMOD,KIZ*NMOD)
    FI=1.0D0/(DEXP((EI-MU)*BETA)+1.0D0)
    MP2_U=MP2_U+2.0D0*BETA*MP2_MU/NKPTOTAL*EI*FI*(1.0D0-FI)
   ENDDO
   ENDDO
   ENDDO
   ENDDO
   ...
   !  ENDIF
    ...
    WRITE(6,'(A,F20.15,A)') 'MU(0)    = ',MP0_MU,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'OMEGA(0) = ',MP0_OMEGA,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'OMEGA(0) = ',MP0_OMEGA1,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'U(0)     = ',MP0_U,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'MU(1)    = ',MP1_MU,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'OMEGA(1) = ',MP1_OMEGA,' HARTREE'
    WRITE(6,'(A,F20.15,A)') 'U(1)     = ',MP1_U,' HARTREE'
    WRITE(*,*) 'S(0) = ', MP0_U - MP0_OMEGA - MP0_MU*NBAR,' HARTREE'
    WRITE(*,*) 'TS(0) = ',(MP0_U - MP0_OMEGA - MP0_MU*NBAR)*BETA,' kb'
    WRITE(*,*) 'S(1) = ', MP1_U - MP1_OMEGA - MP1_MU*NBAR,' HARTREE'
    WRITE(*,*) 'TS(1) = ',(MP1_U - MP1_OMEGA - MP1_MU*NBAR)*BETA,' kb'
    WRITE(*,*) 'MU(2)    = ',MP2_MU,' HARTREE'
    WRITE(*,*) 'OMEGA(2) = ',MP2_OMEGA,' HARTREE'
    WRITE(*,*) 'U(2)     = ',MP2_U,' HARTREE'
    WRITE(*,*) 'U(0) + U(1) = ',MP0_U+MP1_U,' HARTREE'
    WRITE(*,*) 'U(0) + U(1) + U(2) = ',MP0_U+MP1_U+MP2_U,' HARTREE'
    WRITE(*,*) 'OMEGA(0) + OMEGA(1) = ',MP0_OMEGA+MP1_OMEGA,' HARTREE'
    WRITE(*,*) 'OMEGA(0) + OMEGA(1) + OMEGA(2) = ',MP0_OMEGA+MP1_OMEGA+MP2_OMEGA,' HARTREE'
    WRITE(*,*) 'MU(0) + MU(1) = ',MP0_MU+MP1_MU,' HARTREE'
    WRITE(*,*) '-(MU(0) + MU(1))*N = ', -(MP0_MU+MP1_MU)*NBAR, ' HARTREE'
    WRITE(*,*) 'MU(0) + MU(1) + MU(2) = ',MP0_MU+MP1_MU+MP2_MU,' HARTREE'

    WRITE(*,*) 'MP2 OMEGA 1E = ',MP2_OMEGA_1E,' HARTREE'
    WRITE(*,*) 'MP2 OMEGA 2E = ',MP2_OMEGA_2E,' HARTREE'
    WRITE(*,*) 'MP2 OMEGA 4E = ',MP2_OMEGA_4E,' HARTREE'

    WRITE(*,*) 'MP2 MU 1E  = ',MP2_MU_1E,' HARTREE'
    WRITE(*,*) 'MP2 MU 2E  = ',MP2_MU_2E,' HARTREE'
    WRITE(*,*) 'MP2 MU 3E  = ',MP2_MU_3E,' HARTREE'
    WRITE(*,*) 'MP2 MU 4E  = ',MP2_MU_4E,' HARTREE'

    WRITE(*,*) 'MP2 U 1E  = ',MP2_U_1E,' HARTREE'
    WRITE(*,*) 'MP2 U 2E  = ',MP2_U_2E,' HARTREE'
    WRITE(*,*) 'MP2 U 3E  = ',MP2_U_3E,' HARTREE'
    WRITE(*,*) 'MP2 U 4E  = ',MP2_U_4E,' HARTREE'
