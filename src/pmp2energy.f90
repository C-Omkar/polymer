...
!write(*,*) emp2s_tmp,emp2t_tmp
   ...
   IF (MYID == 0) WRITE(6,'(A,F20.15,A)') 'CURRENT EHKFS   = ',EHFKS,' HARTREE'
...
!  ENDIF
   ...
   DO MOB=ICORE+1,IALL(KBX,KBY,KBZ)-IVIRTCORE
      FTDS(MOB,MOB,KBX,KBY,KBZ)=FTDS(MOB,MOB,KBX,KBY,KBZ)-EPSILON(MOB,KBX*NMOD,KBY*NMOD,KBZ*NMOD)
   ENDDO
   ENDDO
   ENDDO
   ENDDO
   ...
   NBAR=NBAR*1.0D0/DFLOAT(MAX(1,2*KVCX)*MAX(1,2*KVCY)*MAX(1,2*KVCZ))
   TEMPORARY=0.0D0
   ERRORTRAP=1.0D-15
