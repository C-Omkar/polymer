...
!  INCLUDE "mpif.h"
   ...
   DOUBLE PRECISION :: DGAP,IGAP,VBM,CBM
...
!   IF (F2 > MAXE) CALL PABORT('DETERMINATION OF FERMI ENERGY FAILED')
   ...
   ENDIF
    ...
    ENDDO
    DGAP=DGAP+IGAP
    IF (MYID == 0) WRITE(*,*) 'BAND GAP          = ',DGAP,' HARTREE (',DGAP*EV,' EV )'
   ...
   ELSE
    IF (TEMP == 0.0D0) THEN
     ...
     WRITE(6,'(A)') 'ZERO CASE: FERMI ENERGY NOT DETERMINED'
     DGAP=1.0D99
     DO IX=-KVCX/NMOD,MAX(0,KVCX/NMOD-1)
     DO IY=-KVCY/NMOD,MAX(0,KVCY/NMOD-1)
     DO IZ=-KVCZ/NMOD,MAX(0,KVCZ/NMOD-1)
      IF (ABS(E(M+1,IX*NMOD,IY*NMOD,IZ*NMOD)-E(M,IX*NMOD,IY*NMOD,IZ*NMOD)) < DGAP) &
       DGAP=ABS(E(M+1,IX*NMOD,IY*NMOD,IZ*NMOD)-E(M,IX*NMOD,IY*NMOD,IZ*NMOD))
     ENDDO
     ENDDO
     ENDDO

     IF (MYID == 0) WRITE(*,*) 'DIRECT BAND GAP   = ',DGAP,' HARTREE (',DGAP*EV,' EV )'
   !   IGAP=1.0D99
   !   DO IX=-KVCX/NMOD,MAX(0,KVCX/NMOD-1)
   !   DO IY=-KVCY/NMOD,MAX(0,KVCY/NMOD-1)
   !   DO IZ=-KVCZ/NMOD,MAX(0,KVCZ/NMOD-1)
   ...
   !   ENDDO
     ...
     DO IY=-KVCY/NMOD,MAX(0,KVCY/NMOD-1)
     DO IZ=-KVCZ/NMOD,MAX(0,KVCZ/NMOD-1)
      IF (MIN(E(M,IX*NMOD,IY*NMOD,IZ*NMOD),E(M+1,IX*NMOD,IY*NMOD,IZ*NMOD)) > VBM) &
       VBM=MIN(E(M,IX*NMOD,IY*NMOD,IZ*NMOD),E(M+1,IX*NMOD,IY*NMOD,IZ*NMOD))
     ENDDO
     ENDDO
     ENDDO
     IGAP=1.0D99
     ...
     DO IY=-KVCY/NMOD,MAX(0,KVCY/NMOD-1)
     DO IZ=-KVCZ/NMOD,MAX(0,KVCZ/NMOD-1)
      IF (MAX(E(M,IX*NMOD,IY*NMOD,IZ*NMOD),E(M+1,IX*NMOD,IY*NMOD,IZ*NMOD)) < IGAP) &
       IGAP=MAX(E(M,IX*NMOD,IY*NMOD,IZ*NMOD),E(M+1,IX*NMOD,IY*NMOD,IZ*NMOD))
     ENDDO
     ENDDO
     ENDDO
     IGAP=IGAP-VBM
     IF (MYID == 0) WRITE(*,*) 'INDIRECT BAND GAP = ',IGAP,' HARTREE (',IGAP*EV,' EV )'
