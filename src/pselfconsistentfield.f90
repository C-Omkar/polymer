...
!  INCLUDE "mpif.h"
   ...
   IF (LOPTN(113)) WRITE(6,'(A)') "ENTERING NON FERMI LEVEL BUILDING BRANCH"
...
!    IF (LKLI) CALL KLI_EXCHANGE
   ...
   IF (IOPTN(9) == 1) CALL DUMP15(P_C,NCGS,CEL1X,CEL1Y,CEL1Z,78)
...
! OLD MPE CODE END
   ...
   IF (DOPTN(108) > 0.0D0) THEN
    ...
    ! - CHEMICAL POTENTIAL x N (sum over all k-points, then average)
    B=0.0D0
    DO QX=-KVCX,MAX(0,KVCX-1)
    DO QY=-KVCY,MAX(0,KVCY-1)
    DO QZ=-KVCZ,MAX(0,KVCZ-1)
     DO J=ICORE+1,IALL(QX,QY,QZ)
      FMINUS=1.0D0/(DEXP((EPSILON(J,QX,QY,QZ)-FERMI)/BOLTZMANN/TEMPHF)+1.0D0)
      B=B-2.0D0*FMINUS*FERMI
     ENDDO
    ENDDO
    ENDDO
    ENDDO
    B=B/DFLOAT(MAX(1,2*KVCX)*MAX(1,2*KVCY)*MAX(1,2*KVCZ))
    IF (CONVERGE.AND.(MYID == 0)) WRITE(6,'(A,F25.14,A)') 'CHEMICAL POTENTIAL CONTRIB  = ',B,' HARTREE'
    EHFKS=EHFKS+B
    ! + kB T (F- ln F- + F+ ln F+) (sum over all k-points, then average)
    B=0.0D0
    DO QX=-KVCX,MAX(0,KVCX-1)
    DO QY=-KVCY,MAX(0,KVCY-1)
    DO QZ=-KVCZ,MAX(0,KVCZ-1)
     DO J=ICORE+1,IALL(QX,QY,QZ)
      FMINUS=1.0D0/(DEXP((EPSILON(J,QX,QY,QZ)-FERMI)/BOLTZMANN/TEMPHF)+1.0D0)
      FPLUS=1.0D0-FMINUS
      IF ((FMINUS > 0.0D0).AND.(FPLUS > 0.0D0)) &
       B=B+2.0D0*(FMINUS*DLOG(FMINUS)+FPLUS*DLOG(FPLUS))*BOLTZMANN*TEMPHF
     ENDDO
    ENDDO
    ENDDO
    ENDDO
