...
!3 ENDIF
   ...
   ALLOCATE(FTDS(NCGS,NCGS,-KVCX:KVCX,-KVCY:KVCY,-KVCZ:KVCZ))
   ALLOCATE(FTDM(NCGS,NCGS))
...
!call pabort('debug stop')
    ...
    ELSE
     IF (DOPTN(98) == 0.0D0) THEN
      ...
      ENDIF
     ELSE
      !    WRITE(6,'(A)') 'REACHED SOLIDS FINITE T'
      !    CALL FOCK_BUILD_ARBITRARY_T_MO
      !    CALL FINITE_MP2_ENERGY_SOLIDS
      ! IF (KVCX == 0 .AND. KVCY == 0 .AND. KVCZ == 0) THEN
      !    WRITE(6,'(A)') 'REACHED MOLECULES FINITE T'
      !    CALL FINITE_MP2_ENERGY
      ! ENDIF

      WRITE (6,'(A)') 'REACHED FINITE T MP2 ENERGY'
      CALL FOCK_BUILD_ARBITRARY_T_MO
      CALL FINITE_MP2_ENERGY
     ENDIF
     IF (IOPTN(44) > 0) THEN
