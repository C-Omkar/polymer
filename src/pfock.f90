...
! SOLVE SECULAR EQUATION BY CANONICAL TRANS-FORMATION OF OVERLAP MATRIX.  RETURNS THE
...
SUBROUTINE FOCK_BUILD_ARBITRARY_T_MO
! BUILDS FOCK MATRIX IN THE MO BASIS FOR ARBITRARY TEMPERATURE FOR SOLIDS AFTER SCF STAGE TO BE USED IN FINITE DIFFERENCE FOCK MATRIX
! STORES CURRENT C_C and X_C IN TEMP VARIABLES, USES RESTORE_FOURINDEXERI TO CREATE NEW DENSITY MATRIX
    ...
    IMPLICIT NONE
    INTEGER :: IX,IY,IZ,QX,QY,QZ,I,J,K,L,N,JJ
    DOUBLE PRECISION :: TEMPMP, FERMI_MP, BETA
    DOUBLE PRECISION :: OCC,W,A,B
    DOUBLE COMPLEX :: PX,PY,PZ,PQX,PQY,PQZ,AVE
    DOUBLE COMPLEX, ALLOCATABLE :: WS(:,:),WF(:,:),WK(:,:),TMP(:,:)
    DOUBLE PRECISION, ALLOCATABLE :: P_SAVE(:,:,:,:,:),P_MP(:,:,:,:,:)
    DOUBLE PRECISION, ALLOCATABLE :: C_C_SAVE(:,:,:,:,:),X_C_SAVE(:,:,:,:,:), F_C_SAVE(:,:,:,:,:)
    DOUBLE PRECISION,ALLOCATABLE :: MPE(:,:,:),W1(:,:)

    ! --- allocate work arrays
    ALLOCATE(WS(NCGS,NCGS),WF(NCGS,NCGS),WK(NCGS,NCGS),TMP(NCGS,NCGS))
    ALLOCATE(P_SAVE(NCGS,NCGS,-CEL1X:CEL1X,-CEL1Y:CEL1Y,-CEL1Z:CEL1Z))
    ALLOCATE(P_MP(NCGS,NCGS,-CEL1X:CEL1X,-CEL1Y:CEL1Y,-CEL1Z:CEL1Z))
    ALLOCATE(C_C_SAVE(NCGS,NCGS,-CEL1X:CEL1X,-CEL1Y:CEL1Y,-CEL1Z:CEL1Z))
    ALLOCATE(X_C_SAVE(NCGS,NCGS,-CEL1X:CEL1X,-CEL1Y:CEL1Y,-CEL1Z:CEL1Z))
    ...
    DO IX=-KVCX,MAX(0,KVCX-1)
     ...
     IF (KVCX == 0) THEN
      PX=DCMPLX(1.0D0,0.0D0)
     ELSE
      PX=DCMPLX(0.0D0,PI*DFLOAT(IX)/DFLOAT(KVCX))
     ENDIF
     IF (KVCY == 0) THEN
      PY=DCMPLX(1.0D0,0.0D0)
     ELSE
      PY=DCMPLX(0.0D0,PI*DFLOAT(IY)/DFLOAT(KVCY))
     ENDIF
     IF (KVCZ == 0) THEN
      PZ=DCMPLX(1.0D0,0.0D0)
     ELSE
      PZ=DCMPLX(0.0D0,PI*DFLOAT(IZ)/DFLOAT(KVCZ))
     ENDIF
     W=1.0D0/DFLOAT(MAX(1,2*KVCX)*MAX(1,2*KVCY)*MAX(1,2*KVCZ))
...
!    WRITE(6,'(A)') '----------------------------------'
    ...
    DEALLOCATE(WS, WF, WK, TMP)
    DEALLOCATE(P_SAVE, P_MP)
    DEALLOCATE(C_C_SAVE, X_C_SAVE, F_C_SAVE)
    DEALLOCATE(MPE, W1)   

    RETURN
END SUBROUTINE FOCK_BUILD_ARBITRARY_T_MO
